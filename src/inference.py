...
def process_audio_file(model, input_file, output_file, device="cpu", chunk_size=None):
    ...
    if audio.ndim == 2:
        print(f"    [!] Stereo file detected, converting to mono (averaging channels)")
...
def main():
    parser = argparse.ArgumentParser(description="Run inference with trained amp model")
    ...
    parser.add_argument("-c", "--chunk-size", type=int, default=None,
                       help="Process in chunks of N samples (for large files)")
    ...
    if args.output is None:
        ...
        args.output = str(input_path.parent / f"{input_path.stem}_processed{input_path.suffix}")

    # Process audio
    process_audio_file(model, args.input, args.output, device, args.chunk_size)
